...
// Constructor
Country__continentcountries::Country__continentcountries(
    ContinentQuery *operation, const std::shared_ptr<Country> &inst)
...
std::shared_ptr<Country> deserializers::des_Country__continentcountries(
    const QJsonObject &data, const ContinentQuery *operation) {
  if (data.isEmpty()) {
    return {};
  }
...
// Updater
void updaters::update_Country__continentcountries(
    const std::shared_ptr<Country> &inst, const QJsonObject &data,
    const ContinentQuery *operation) {
  if (!data.value("capital").isNull()) {
...
void Country__continentcountries::qtgql_replace_concrete(
    const std::shared_ptr<Country> &new_inst) {
  if (new_inst == m_inst) {
    return;
  }
...
// Constructor
Continent__continent::Continent__continent(
    ContinentQuery *operation, const std::shared_ptr<Continent> &inst)
    : m_inst{inst}, ObjectTypeABC ::ObjectTypeABC(operation) {
  m_operation = operation;

  auto init_vec_countries = std::vector<Country__continentcountries *>();
  for (const auto &node : m_inst->get_countries()) {
    init_vec_countries.push_back(
        new Country__continentcountries(operation, node));
  }
  m_countries = new qtgql::bases::ListModelABC<Country__continentcountries *>(
      this, std::move(init_vec_countries));
...
void Continent__continent::_qtgql_connect_signals() {
  ...
  connect(m_inst_ptr, &Countries::Continent::countriesChanged, this, [&]() {
    ...
    for (int i = 0; i < new_len; i++) {
      ...
      if (i >= prev_len) {
        m_countries->append(
            new Country__continentcountries(operation, concrete));
      } else {
        ...
        if (proxy_to_update) {
          ...
          m_countries->replace(
              i, new Country__continentcountries(operation, concrete));
        }
      }
    }
  });
...
deserializers::des_Continent__continent(const QJsonObject &data,
                                        const ContinentQuery *operation) {
  if (data.isEmpty()) {
    return {};
  }
  ...
  if (!data.value("countries").isNull()) {
    ...
    for (const auto &node : data.value("countries").toArray()) {
      ...
      countries_init_vec.push_back(
          deserializers::des_Country__continentcountries(node.toObject(),
                                                         operation));
    };
...
// Updater
void updaters::update_Continent__continent(
    const std::shared_ptr<Continent> &inst, const QJsonObject &data,
    const ContinentQuery *operation) {
  if (!data.value("name").isNull()) {
...
[[nodiscard]] const qtgql::bases::ListModelABC<Country__continentcountries *> *
Continent__continent::get_countries() const {

  return m_countries;
};

void Continent__continent::qtgql_replace_concrete(
    const std::shared_ptr<Continent> &new_inst) {
  if (new_inst == m_inst) {
    return;
  }
  m_inst->disconnect(this);
...
// Constructor
Query__::Query__(ContinentQuery *operation)
  ...
  if (m_inst->get_continent({operation->vars_inst.code.value()})) {
    m_continent = new Continent__continent(
        operation, m_inst->get_continent({operation->vars_inst.code.value()}));
  }
...
void updaters::update_Query__(Query *inst, const QJsonObject &data,
                              const ContinentQuery *operation) {
  std::tuple<qtgql::bases::scalars::Id> m_continent_args = {
      operation->vars_inst.code.value()};
  if (!inst->m_continent.contains(m_continent_args))

  {
    if (!data.value("continent").isNull()) {
      inst->set_continent(deserializers::des_Continent__continent(
                              data.value("continent").toObject(), operation),
                          {operation->vars_inst.code.value()});
    };
  } else if (!data.value("continent").isNull()) {

    auto continent_data = data.value("continent").toObject();

    updaters::update_Continent__continent(
        inst->m_continent.at(m_continent_args), continent_data, operation);
