...
des_Country__continentcountries(const QJsonObject &data,
                                const ContinentQuery *operation);
std::shared_ptr<Continent>
des_Continent__continent(const QJsonObject &data,
                         const ContinentQuery *operation);
}; // namespace deserializers

namespace updaters {
void update_Country__continentcountries(const std::shared_ptr<Country> &inst,
                                        const QJsonObject &data,
                                        const ContinentQuery *operation);
void update_Continent__continent(const std::shared_ptr<Continent> &inst,
                                 const QJsonObject &data,
                                 const ContinentQuery *operation);
...
public:
  Country__continentcountries(ContinentQuery *operation,
                              const std::shared_ptr<Country> &inst);
...
class Continent__continent : public qtgql::bases::ObjectTypeABC {
  ...
  Q_PROPERTY(const qtgql::bases::ListModelABC<Country__continentcountries *>
                 *countries READ get_countries NOTIFY countriesChanged);
...
public:
  Continent__continent(ContinentQuery *operation,
                       const std::shared_ptr<Continent> &inst);
...
class Query__ : public qtgql::bases::ObjectTypeABC {
  ...
  Q_PROPERTY(const Continent__continent *continent READ get_continent NOTIFY
                 continentChanged);
...
public:
  ...
  ContinentQuery()
      : qtgql::bases::OperationHandlerABC(
            qtgql::bases::GraphQLMessage("query ContinentQuery($code: ID!) {"
                                         "  continent(code: $code) {"
                                         "    name"
                                         "    countries {"
                                         "      capital"
...
public:
  ...
  UseContinentQuery(QObject *parent = nullptr) : QObject(parent) {
    ...
    connect(op_ptr, &ContinentQuery::dataChanged, this,
            [&] { emit dataChanged(); });
    connect(op_ptr, &ContinentQuery::completedChanged, this,
            [&] { emit completedChanged(); });
    connect(op_ptr, &ContinentQuery::operationOnFlightChanged, this,
